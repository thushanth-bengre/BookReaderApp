 ...
 * @author Minal
 */
public class SellerBiddingPanel extends javax.swing.JPanel {

    /**
     * Creates new form SellerBiddingPanel
     */
    
    private JPanel userProcessContainer;
    private BookOfferCampaign campaign;
    ...
    SellerBiddingPanel(JPanel userProcessContainer, BookOfferCampaign campaign, CampaignManager organization, UserAccount account, EcoSystem business,Enterprise enterprise) {
        initComponents();
        ...
        jLabelCampName.setText("Book: " + campaign.getBook().getName() + "      Release Date: " + campaign.getDiscount());
        result = getList();
        populateTable();
    }
                            ...
                            for(UserAccount ua: o.getUserAccountDirectory().getUserAccountList()){
                                if(ua instanceof Readers){
                                    for(ShelfItem si:((Readers) ua).getShelf().getShelfList()){
                                        if(si != null){
                                            if(si.getTitle() == campaign.getBook()){
                                                if(si.getStatus() == ShelfItem.Status.ToRead){
                                                    list.add((Readers) ua);
                                                }                                                
                                            }
                                        }
    ...
    public void populateTable(){
        ...
        for(Readers r: result){
            //f(!enterprise.getName().equals(r.getAd().getNbc().getPubName())){
            Object[] row = new Object[3];
            row[0]= r;            
            row[1] = r.getAd().getBiddingPriceboc();
    ...
    private void initComponents() {
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Reader", "Current Bidding price", "No Of books in Shelf"
            }
        ) {
    ...
    private void btnBidActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBidActionPerformed
        ...
        if(selectedRowValue >= 0){
            ...
            if(r.getAd().getBiddingPriceboc() >0){
            BidOverRideNotification bon = (BidOverRideNotification) r.getAd().getBoc().getOrganization().getWorkQueue().createBidOverRideNotification();
            bon.setStatus("New");
            bon.setSender(account);
            bon.setMessage("Your bid for Reader " + r + " on book " + r.getAd().getBoc().getBook() +  " has been overridden. Bid again for an advertisement space!");
            }
            r.getAd().setBoc(campaign);
            r.getAd().increaseBOCBid();
